...
public class InstallActivity extends AppCompatActivity {
    ...
    private void installXapk() {
        ...
        if (UtilKSysRom.isMeizu() || UtilKSysRom.isVivo()) {
    ...
    private void addApkToInstallSession(String filePath, PackageInstaller.Session session)
        ...
        try (OutputStream packageInSession = session.openWrite(UtilKStrFile.getStrFileName(filePath)/*getFileName()*/, 0, new File(filePath).length());
